...
/*
	Rollup.js v0.36.1
	Wed Sep 28 2016 08:49:20 GMT-0400 (EDT) - commit 5becfeb86af3c9090bbf2a5bdc7619698e8d3fda
...
function first ( candidates ) {
	return function () {
		...
		return candidates.reduce(
			function ( promise, candidate ) {
				return promise.then(
					function (result) {
						return result != null ? result : Promise.resolve(
							candidate.apply( void 0, args )
						);
					}
				);
		}, Promise.resolve() );
...
function assignToForLoopLeft ( node, scope, value ) {
	if ( node.type === 'VariableDeclaration' ) {
		var values = node.declarations[0].proxies.values();
		for ( var i = 0; i < values.length; i++ ) {
			var proxy = values[i];
...
function enhanceNode ( raw, parent, module, code ) {
	...
	for ( var i = 0; i < keys$1[ raw.type ].length; i++ ) {
		var key = keys$1[ raw.type ][i];
...
var hypothetical = function rollupPluginHypothetical(options) {
  options = options || {};
...
[{
			resolveId: function(importee, importer) {
				...
				console.log("Importing", importee, importer);
			...
			"load": function(id) {
				console.log("Loading", id, id in modules, modules[id]);
				
				return {code: modules[id], map: null};
				return modules[id];
			}
		}]
...
var do_rollup = function do_rollup(entry, modules, options) {
	var result = {done: false, warnings: []};
	...
	var prom = rollup({
		...
		plugins: [
			hypothetical({
				leaveIdsAlone: true,
				impliedExtensions: ['.js', '.py'],
				cwd: '/',
				files: modules
			})
		],
	}).then(function(bundle){
			result.imports = bundle.imports;
			result.exports = bundle.exports;
			
			console.log('before-generate');
			result.code = bundle.generate(options).code;
			console.log('after-generate', result.code);
