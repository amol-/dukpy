...
class TestPackageInstaller(unittest.TestCase):
    ...
    def test_install_command(self):
        with mock.patch.object(
            sys, "argv", ["dukpy-install", "react", "0.14.8", "-d", self.tmpdir]
        ):
            dukpy_install.main()
    ...
    def test_install_command_latest_ver(self):
        with mock.patch.object(
            sys, "argv", ["dukpy-install", "react", "-d", self.tmpdir]
        ):
            dukpy_install.main()
        assert os.path.exists(os.path.join(self.tmpdir, "react"))
    ...
    def test_install_scoped_package(self):
        with mock.patch.object(
            sys, "argv", ["dukpy-install", "@reactivex/rxjs", "5.0.0-beta.11"]
        ):
            dukpy_install.main()
    ...
    def test_install_command_substrate_error(self):
        with mock.patch.object(
            sys, "argv", ["dukpy-install", "react", "9999", "-d", self.tmpdir]
        ):
    ...
    def test_install_unexisting_package(self):
        with self.assertRaises(Exception) as err:
            dukpy.install_jspackage(
                "non_existing_suerly_missing_dunno", "1", self.tmpdir
            )
    ...
    def test_install_missing_download_url(self):
        with mock.patch(
            "dukpy.install._fetch_package_info",
